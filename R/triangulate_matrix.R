...
triangulate_matrix = function(
  ...
  #Switch axes (optional)
  if (y_up) {
    full_vertices = full_vertices[, c(1, 3, 2, 4)]
  } else {
    full_vertices = full_vertices[nrow(full_vertices):1, ]
  }
  if (verbose) {
    base_triangles = ncol(heightmap) * nrow(heightmap) * 2
    message(sprintf(
      "%0.1f%% reduction: Number of triangles reduced from %d to %d. Error: %f",
      (1 - (nrow(full_vertices) / 3) / base_triangles) * 100,
      base_triangles,
      nrow(full_vertices) / 3,
      error
    ))
  }
