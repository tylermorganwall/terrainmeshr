...
Triangulator::Triangulator(const Heightmap &heightmap) :
    ...
    const std::vector<int> ind_y)
{
  ...
  // for(int i = 0; i < ind_x.size(); i++) {
  //   const int p1 = AddPoint(ivec2{ind_x[i], ind_y[i]});
  // }
  const int x0 = 0;
  const int y0 = 0;
  const int x1 = m_Heightmap.Width() - 1;
  const int y1 = m_Heightmap.Height() - 1;
  const int p1 = AddPoint(ivec2{x1, y0});
...
std::vector<vec3> Triangulator::Points() const {
  ...
  for (const ivec2 &p : m_Points) {
    ...
    pts.x = p.x;
    pts.y = h1 - p.y;
    pts.z = m_Heightmap.At(p.x, p.y);
    points.emplace_back(pts);
  }
  return points;
}

std::vector<ivec3> Triangulator::Triangles() const {
  std::vector<ivec3> triangles;
  triangles.reserve(m_Queue.size());
  for (const int i : m_Queue) {
    ivec3 tri;
    tri.x = m_Triangles[i * 3 + 0];
    tri.y = m_Triangles[i * 3 + 1];
    tri.z = m_Triangles[i * 3 + 2];
...
int Triangulator::AddPoint(const ivec2 point) {
    ...
    int e)
{
  if (e < 0) {
    ...
    ivec2 temp{0,0};
    m_Candidates.emplace_back(temp);
